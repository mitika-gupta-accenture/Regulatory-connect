...
import { continueGenerateDossierID } from '../action';
...
export default async function Page({ params, searchParams }: Props) {
  const clearDossier = await session.get('clearDossierId');

  if (clearDossier) {
    await clearOnlyAnswers();
    await session.set('clearDossierId', false);
  }
    ...
    return (
      <>
        ...
        {identifier !== 'generation-successful' && (
          <BackButton previousPage="/choose-submission-type" />
        )}
        <Heading
        ...
        <Summary answers={allAnswers} formPath="generate-dossier-id" />
        <QuestionBuilder
          question={question}
          previousPageAnswers={previousPageAnswers}
          files={files}
          apiData={apiData}
          serverAction={continueGenerateDossierID}
        />
      </>
    );
  } else if (question) {
